...
|      enum       |                      `Array`&#124;`Object`                       |  `N`  |   `-`   |                                                                       单选时的选项值列表，仅当`type`为`string`、`number`、`range`、`address`时支持。当enum为对象类型时，与[List](./component/list/index.md)组件的schema一致，表示从接口中获取选项。为数组时表示选项列表是已知的，如下                                                                        |
