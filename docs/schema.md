...
|    enum     |                             `Array`                              |  `N`  |   `-`   |                                                                                                                                         单选时的选项值列表，仅当`type`为`string`、`number`、`range`、`address`时支持                                                                                                                                         |
|  enumNames  |                             `Array`                              |  `N`  |   `-`   |                                                                                                                                                                    `enum`选项值对应的文案                                                                                                                                                                    |
...
|     名称      |           类型           | 必填  | 默认值 |                                                                                                             描述                                                                                                             |
| :-----------: | :----------------------: | :---: | :----: | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
|    format     |         `String`         |  `N`  |  `-`   |                                                               不设置表示普通输入字符串；`color`表示颜色；`image`表示图片；`video`表示视频；`file`表示普通文件                                                                |
...
|  urlFetcher   |        `Function`        |  `N`  |  `-`   |                                         文件上传后的url提取函数，当`format`为`image`、`video`、`file`时有效，接受`(response)`参数，`response`为服务器响应数据，函数返回值为文件的url                                         |
|   component   | `String`&#124;`Function` |  `N`  |  `-`   |   `format`未设置时默认使用`input`组件，当`component`为`textarea`时使用`textarea`组件；当设置了`enum`属性时，默认使用`radio`组件，但当`format`为`color`或未设置时，可以将`component`设置成`select`，此时将使用`select`组件    |
...
| valueFormat |         `String`         |  `Y`  |    `-`     |                                                                                          表单项值的格式，与`format`配合使用，与`element-ui`的[DatePicker](https://element.eleme.cn/#/zh-CN/component/date-picker)组件的日期格式一致                                                                                           |
|  component  | `String`&#124;`Function` |  `N`  |    `-`     |                                                                                                                当设置了`enum`属性时，默认使用`radio`组件，将`component`设置成`select`，此时将使用`select`组件                                                                                                                 |
