...
|   uploader    |        `Function`        |  `N`  |  `-`   |    当`format`未设置且`component`为`richtext`时有效，如果未设置`uploader`会禁止富文本上传文件，设置成函数时可上传文件，接受`(blob, success, fail)`三个参数，`blob`表示文件信息，上传成功后需要执行`success(location)`，`location`表示文件上传后的地址，失败后需要执行`fail()`    |
