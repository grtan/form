    ...
    <div class="fm-list__header">
      <div class="fm-list__search">
        <template v-if="searchProps.length">
          <div v-for="prop in searchProps" :key="prop" class="fm-list__search-item">
            {{ fixedSchema.properties[prop].title }}
            <v-base v-model="search[prop]" :schema="fixedSchema.properties[prop]" />
          </div>
          <el-button @click="setSearch()"> 重置 </el-button>
        </template>
      </div>
